...
try:
    ...
    from paddlenlp.transformers.mgen.modeling import MGenPreTrainedModel as MGenPretrainedModel
...
_CHECKPOINT_FOR_DOC = "mgen"
_CONFIG_FOR_DOC = "MGenConfig"
MGen_PRETRAINED_MODEL_ARCHIVE_LIST = ["mgen-7b"]
...
class MGenLMHeadModel(MGenPretrainedModel):
    ...
    def chat_batch(
        ...
        for q in query:
            ...
            raw_text_len_list.append(len(raw_text))
            context_length_list.append(len(context_tokens))
            input_id = paddle.to_tensor(data=[context_tokens])
            input_ids.append(input_id)
        input_ids = paddle.concat(input_ids, axis=0)
        outputs, _ = self.generate(
            input_ids, logits_processors=logits_processors, generation_config=generation_config, use_fast=True, use_fp16_decoding=True, early_stopping=True, **kwargs
        )

        outputs = paddle.concat(x=[input_ids, outputs], axis=1)
        responses = []
        for output, rt_len, c_length in zip(outputs, raw_text_len_list, context_length_list):
            response = decode_tokens(
                output,
                tokenizer,
                raw_text_len=rt_len,
