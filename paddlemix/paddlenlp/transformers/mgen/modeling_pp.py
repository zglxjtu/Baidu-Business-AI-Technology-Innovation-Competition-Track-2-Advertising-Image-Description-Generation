...
class MGenEmbeddingPipe(nn.Layer):
    """Extends MGenEmbeddings to forward attention_mask through the pipeline."""

    def __init__(self, config):
        super(MGenEmbeddingPipe, self).__init__()
    ...
    def forward(self, args):
        ...
        if attention_mask is not None:
            attention_mask = MGenModel._prepare_decoder_attention_mask(
...
class MGenRMSNormPipe(MGenRMSNorm):
    def forward(self, args):
        hidden_states, attention_mask, position_ids = parse_args(args)
        return super().forward(hidden_states)


class MGenForCausalLMPipe(PipelinePretrainedModel, PipelineLayer):
    """MGenForPretraining adapted for pipeline parallelism.

    The largest change is flattening the MGenModel class so we can express it as a
    sequence of layers including embedding, transformer layers, and output.
    """

    config_class = MGenConfig

    _get_tensor_parallel_mappings = MGenPretrainedModel._get_tensor_parallel_mappings
    _init_weights = MGenPretrainedModel._init_weights
    _keys_to_ignore_on_load_unexpected = MGenPretrainedModel._keys_to_ignore_on_load_unexpected
    ...
    def __init__(self, config):
        ...
        self.add_sequential_layer(LayerDesc(MGenEmbeddingPipe, config=config), "qwen")
        for i in range(config.num_hidden_layers):
            self.add_sequential_layer(
                LayerDesc(MGenBlockPipe, config=config),
                f"qwen.h.{i}",
            )
        self.add_sequential_layer(LayerDesc(MGenRMSNormPipe, config=config), "qwen.ln_f")
        self.add_sequential_layer(LayerDesc(MGenLMHead, config=config), "lm_head")
        ...
        PipelineLayer.__init__(
            ...
            loss_fn=MGenPretrainingCriterion(config),
