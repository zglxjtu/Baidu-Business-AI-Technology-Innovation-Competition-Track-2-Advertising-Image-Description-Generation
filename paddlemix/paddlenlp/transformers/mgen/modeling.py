...
from .configuration import MGenConfig
...
class MGenBlock(nn.Layer):
    def __init__(self, config):
        super().__init__()
        self.ln_1 = MGenRMSNorm(config)
        self.attn = MGenAttention(config)
        self.ln_2 = MGenRMSNorm(config)
        self.mlp = MGenMLP(config)
...
class MGenPretrainedModel(PretrainedModel):
    config_class = MGenConfig
    base_model_prefix = "mgen"
    ...
    @classmethod
    def _get_name_mappings(cls, config: MGenConfig) -> List[StateDictNameMapping]:
        mappings = [
            "wte.weight",
            "ln_f.weight",
        ]
        ...
        if config.architectures is not None:
            if "MGenForCausalLM" in config.architectures or "MGenLMHeadModel" in config.architectures:
                mappings.extend(
                    [
                        [
                            "lm_head.weight",
...
class MGenModel(MGenPretrainedModel):
    def __init__(self, config):
...
class MGenLMHead(nn.Layer):
    def __init__(self, config: MGenConfig):
        super(MGenLMHead, self).__init__()
        self.config = config
        if config.tensor_parallel_degree > 1:
...
class MGenPretrainingCriterion(paddle.nn.Layer):
    """
    Criterion for Llama.
    ...
    def __init__(self, config):
        ...
        super(MGenPretrainingCriterion, self).__init__()
...
class MGenForCausalLM(MGenPretrainedModel):
    _keys_to_ignore_on_load_missing = [r"h\.\d+\.attn\.rotary_emb\.inv_freq"]

    def __init__(self, config):
        super().__init__(config)
        self.qwen = MGenModel(config)
        self.lm_head = MGenLMHead(config)
        self.criterion = MGenPretrainingCriterion(config)
...
MGenLMHeadModel = MGenForCausalLM
