...
from models import MGenVLTokenizer
from auto import AutoConfigMIX, AutoModelMIX, AutoProcessorMIX, AutoTokenizerMIX
from datasets import MixDataset, MIXTokenMapDataset
from trainer import (
    DataArgument,
    GenerateArgument,
    ModelArgument,
...
def main():
    ...
    if isinstance(tokenizer, MGenVLTokenizer):
    ...
    if data_args.mixtoken:
        if (
            model.base_model_prefix not in ["llava", "mgen"]
            and training_args.pipeline_parallel_degree < 1
        ):
            raise NotImplementedError("MIXToke data stream is only implemented for llava MGen so far.")
    ...
    trainable_list = ["transformer.llm.h.31", "transformer.llm.h.30", "transformer.llm.h.29", "transformer.llm.h.28", "transformer.llm.h.27", "transformer.llm.h.26", "transformer.llm.h.25", "transformer.llm.h.24", "transformer.llm.h.23", "transformer.llm.h.22", "transformer.llm.h.21", "transformer.llm.h.20", "transformer.llm.h.19", "transformer.llm.h.18", "transformer.llm.h.17", "transformer.llm.h.16"]
    ...
    for name, param in model.named_parameters():
        if not any(nd in name for nd in ambrose_trainable_list):
            param.stop_gradient = True
        else:
            print("[ambrose debug] trainable layer：:", name)
    ...
    if training_args.do_train:
        ...
        #checkpoint = '/data/wsc/adImageCaption/checkpoints/mgen_vl_sft_ckpts_clip/checkpoint-58000/'
