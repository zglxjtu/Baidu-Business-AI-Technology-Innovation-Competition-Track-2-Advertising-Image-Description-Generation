...
from processors.base_processing import ProcessorMixin
from processors.processing_utils import (
    BaseAudioProcessor,
    BaseImageProcessor,
    BaseTextProcessor,
)
...
def get_processor_mapping():
    ...
    for file_name in os.listdir(processors_dir):
        ...
        model_name = None
        if "mgen_vl" in file_name:  
            model_name = "mgen_vl"
        else:
            model_name = file_name.split("_")[0]

        model_module = import_module(f"processors.{file_name[:-3]}")
