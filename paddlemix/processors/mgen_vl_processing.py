...
__all__ = [
    "MGenVLProcessor",
    "MGenVLImageProcessor",
]


class MGenVLProcessor(ProcessorMixin):

    attributes = ["tokenizer"]
    tokenizer_class = "MGenTokenizer"

    def __init__(self, tokenizer, **kwargs):
        super().__init__(tokenizer)
        self.image_start_id = kwargs.get("image_start_id", 151857)
        self.max_len = kwargs.get("max_len", 2048)
        self.image_processor = MGenVLImageProcessor()
    ...
    def train_preprocess(self, sources, system_message: str = "You are a helpful assistant."):
        ...
        image_path = []
        if isinstance(sources, dict) and "image" in sources.keys():
            image_path.append(sources["image"])
        if isinstance(sources, dict) and "conversations" in sources.keys():
            sources_conversations = sources["conversations"]
        #if "<img>" in sources:
        #    result = image_pattern.findall(sources)
        #    for ele in result:
        #        image_path.append(ele[5:-6])
        
        input_id_conversation = self.tokenizer(sources_conversations).input_ids
...
class MGenVLImageProcessor(BaseImageProcessor):
    def __init__(
        self,
        image_size: int = 448,
    ...
    def __call__(self, image_paths: List[str]):
        ...
        for image_path in image_paths:
            ...
            if image_path.startswith("http://") or image_path.startswith("https://"):
            ...
            elif os.path.isfile(image_path) and os.path.splitext(image_path)[1].lower() in \
                ...
                image_buffer = BytesIO(byte_data)
                try:
